...
`
  ...
  datamodel.models.forEach(async (modelInfo) => {
    ...
    const objectFields = modelInfo.fields.filter(
      (field) => field.kind === 'object',
    )
    const otherFields = modelInfo.fields.filter(
      (field) => field.kind !== 'object',
    )
    ...
    objectFields.forEach((field) => {
      ...
      if (!field.isList) {
        const fromFieldName = field.relationFromFields
          ? field.relationFromFields[0]
          : ''
        const fromField = otherFields.find(
          (f) => f.name === fromFieldName || f.dbName === fromFieldName,
        )
    ...
    otherFields.forEach((field) => {
      ...
      if (field.kind === 'scalar') {
        ...
        modelDiagram += `\t\t${parts.join(' ')}\n`
        let relationship = `\t${modelName} }o--|| ${typeToDBName.get(
          field.type,
        )} : "enum:${fieldName}"\n`
        relationships.push(relationship)
      }
